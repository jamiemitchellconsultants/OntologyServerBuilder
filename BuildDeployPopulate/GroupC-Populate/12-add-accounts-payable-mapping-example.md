# Prompt C-12 — Add the accounts-payable mapping example

Using the Brightflag concepts and governed source example from Prompt C-01, mapping instructions
from Prompt C-02, the bounded intake analysis from Prompt A-31, release visibility from Prompt B-05,
and named mapping tools from Prompt A-32, add one synthetic, reviewed accounts-payable example in
...
Read Prompts C-01–C-02, A-21–A-31, B-05, and A-32, the approved qualified-user intake plan, `AGENTS.md`, Project
...
ontology source. Prompt C-01 supplies governed Brightflag OpenAPI source concepts only; it neither
...
and pay-site-reference fields; and bounded example metadata needed by Prompt A-21's registration
submission contract. Use a source kind of `mcp`, its own extractor/provenance, and the catalog's
own digest.
It may reuse governed field concepts and stable references from Prompt C-01 solely for this
...
entity and its input schema to Prompt C-01's governed Brightflag Invoice stable ID. Record both source
...
Resolve every blocker retained by Prompt C-02 with explicit synthetic source facts and reviewed,
...
approved synthetic-source adapter, specifically Prompt C-01's Brightflag Invoice stable ID, target
...
entity. Keep the definition governed data for Prompt A-32's restricted evaluator; do not add
...
one reviewed pay-site reference to beneficiary. It returns Prompt A-32's deterministic provenance
...
the tool all use Prompt C-01's exact Brightflag Invoice stable ID.
...
- The adapter target, mapping-instruction source, mapping-tool governed source, and canonical input
  record all use Prompt C-01's exact Brightflag Invoice stable ID. The pure mapping tool receives that
