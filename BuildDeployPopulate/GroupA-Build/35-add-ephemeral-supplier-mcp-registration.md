# Prompt A-35 — Add ephemeral supplier MCP discovery and registration

Using the qualified-user intake tools from Prompt A-23 and the engineer workbench from Prompt A-24,
...
Prompt A-34 remains an independent, supported alternative and must keep working unchanged. Its skill
captures a catalog in the user's own MCP client and submits normalized evidence through the Prompt A-23 tools. This stage adds a server-connected path; it does not replace, edit, or auto-install that
skill.

This stage depends on Prompts A-18–A-19 and A-23–A-24. Adapt paths and types to the repository as it
...
Do not edit Prompt A-13 or any other earlier Builder prompt. Those stages are historical inputs that
...
- **Prompt A-16:** its preparation tool accepts caller-extracted normalized metadata and performs no
...
- **Prompt A-21:** its plan states that the runtime never retrieves an interface source. The direct
...
- **Prompt A-22:** its intake foundation performs no source retrieval. The new workflow is the only
...
- **Prompt A-23:** its two public submission handlers never fetch a locator, receive raw bytes, or
  ...
  evidence internally, and then use Prompt A-23's canonical submission builder.
- **Prompt A-24:** its original-artifact workflow requires an engineer to supply a separate local
...
- **Prompt A-34:** its skill requires the user's own client to connect, capture, hash, normalize, and
  submit without server-side retrieval. Preserve the skill byte-for-byte. Prompt A-35 adds a separate
  server-connected alternative and does not reinterpret Prompt A-34.

Prompt A-33 remains the historical audit of the boundary that existed before this stage; do not edit
its report or represent it as having audited Prompt A-35. Preserve every requirement from every
earlier prompt unless this section identifies the exact conflicting behavior and replacement.

## Read before editing

Read the canonical repository agent instructions; authentication and capability code; Prompt A-23's
preparation/submission contracts; Prompt A-24's export and event contracts; HTTP transport and host
...
Read `skills/register-supplier-mcp-server/SKILL.md` if Prompt A-34 has been applied. Preserve it
...
- a caller-generated idempotency key under Prompt A-23's existing contract.
...
- the Prompt A-23 receipt shape: opaque ID, payload digest, received timestamp, and `received` status.
...
the Prompt A-23 canonical system-registration builder and `IntakeStore`, preserving Prompt A-23's
...
`ontology_submit_system_registration` schema and Prompt A-34 skill must remain compatible; their
...
Extend the Prompt A-24 engineer export path, still requiring `ontology:intake:review`, so a direct
...
Do not edit Prompt A-13. Update the built `OntologyService` repository's current canonical `AGENTS.md`
...
State the compatibility relationship plainly: Prompt A-34 remains the no-outbound, client-captured
option, while Prompt A-35 is the direct, ephemeral-credential option.
...
- existing Prompt A-23 submissions and the Prompt A-34 skill path remaining compatible;
...
- Prompt A-34's installed skill and existing Prompt A-23 tool schemas remain compatible and unchanged.
