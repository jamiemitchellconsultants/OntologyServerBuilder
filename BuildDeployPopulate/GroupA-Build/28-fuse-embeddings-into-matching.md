# Prompt A-28 — Fuse embeddings into governed matching

Implement only Prompt 4, "Fuse embeddings into the matcher (disabled by default)", from the
current `docs/step4.md` in the separate `OntologyService` repository. Prompts A-25–A-27 have supplied
...
typed contracts from Prompt A-26.
...
Prompt A-26 rather than a reason string. Evidence must agree exactly with the injected context and be
