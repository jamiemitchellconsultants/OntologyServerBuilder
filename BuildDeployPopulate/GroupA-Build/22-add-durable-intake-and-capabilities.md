# Prompt A-22 — Add durable intake and capability authorization

Using the approved qualified-user intake plan from Prompt A-21, implement the first intake-plane
foundation in the separate `OntologyService` repository. Do not change `OntologyServerBuilder` in
this stage. Prompt A-20 remains active and its authentication failure distinction must be preserved.
...
Read the Prompt A-21 plan and inspect the current authentication middleware, verified-principal
contract, configuration, server startup, proposal workflows, tests, deployment material, homelab
guide, Prompt B-03 AWS baseline, and Project Narrative rules. Adapt names and paths to the repository
...
Preserve every existing authentication mode and the Prompt A-20 distinction between invalid tokens
...
whole. Document that this adapter may be enabled under Prompt B-03's multi-instance AWS baseline,
...
- The homelab recovery guidance covers a LocalStack-compatible endpoint override, bucket versioning,
  access scope, encryption, lifecycle rules, and capacity; the Prompt B-03 AWS multi-instance baseline
