# Prompt A-23 — Add qualified-user intake submissions

Using the durable `IntakeStore` and capability authorization from Prompt A-22, add the two
...
plane. Preserve the existing read-only preparation tools from Prompt A-16; the submit tools consume
...
Read Prompt A-21's approved intake plan; Prompt A-22; the existing proposal-preparation tools and their
...
Define and export the canonical submission types and receipt schema that Prompt A-24 will consume.
...
Use the existing Unicode NFC filename canonicalization and structural filename policy from Prompt A-17. Reject path separators, path-like names, empty names, control characters, deceptive directional
