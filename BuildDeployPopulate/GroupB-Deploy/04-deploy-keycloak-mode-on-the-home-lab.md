# Prompt B-04 — Make the home-lab deployment actually run `keycloak` mode
...
> Prompt A-19's contract, with `keycloak` as its default mode.
>
...
`keycloak` mode added in Prompt A-19 can be deployed as shipped, rather than by hand-editing the
...
This stage depends on Prompt B-02 (the home-lab deployment) and Prompt A-19 (the `keycloak` mode and the
OAuth challenge).

## Why this stage exists

Prompt A-19 made `keycloak` the ordinary home-lab mode and left the deployment behind. What is checked
...
The result is a documented mode with no shipped way to run it. Prompt A-19 recorded the required edits
...
- `src/auth.ts` and `src/server.ts` as they stand after Prompt A-19 — in particular which paths are
...
than that: the two well-known paths Prompt A-19 serves, plus the existing `/health` and `/mcp`.
