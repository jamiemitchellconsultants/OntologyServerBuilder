# Prompt 31 — Deploy the service to AWS for production use
...
Prompt 9, and the in-process Entra validation from Prompt 20. It does not depend on Prompts 21–29.

Read before editing, and adapt every name and path below to the repository's current structure:

- `Dockerfile`, `.env.example`, and the homelab deployment added in Prompt 30;
...
**This is the live deployment.** Prompt 30 could label an untested control a manual gate and move
...
- ECS on Fargate running the Prompt 7 image, pulled from ECR by immutable digest, built for
...
- proves the effective task definition carries every hardening control Prompt 7 and Prompt 30
...
reported as passing — Prompt 30's rule, and it does not relax because this environment matters more.
