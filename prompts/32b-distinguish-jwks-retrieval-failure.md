# Prompt 32b — Tell a key-retrieval failure apart from an invalid token
...
This stage depends on Prompt 20 (the verifier chain) and Prompt 32 (`keycloak` mode).

## What this stage is not

It is not a deployment change, and **nothing under `deploy/` may be touched**.

Prompt 32a assumed `deploy/homelab/` describes the deployment and was withdrawn for it. The real
...
- Apply the same treatment to **both** JWT modes. `entra` has the identical defect for the identical
  ...
  Prompt 20's error behaviour, and the Narrative should say so rather than presenting it as
  additive.
