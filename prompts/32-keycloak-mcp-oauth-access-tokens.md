# Prompt 32 — Accept Keycloak access tokens and answer the MCP OAuth challenge
...
This stage depends on the in-process authentication from Prompt 20 and the deployment path from
Prompt 30.

## What this supersedes

Prompt 30 states that `MCP_AUTH_MODE=static` is *the only acceptable homelab mode*. That is no
longer true, and this stage replaces it: `keycloak` becomes the ordinary home-lab mode and `static`
becomes the fallback. Prompt 20's mode table is extended, not rewritten — `static`, `entra` and
`none` keep their current behaviour exactly, including the fail-closed rule for an unset or unknown
mode. Do not edit Prompts 20 or 30; this prompt is the later instruction and says so.
...
- `src/auth.ts`, or whatever module Prompt 20 created for the verifier chain and the bearer
  middleware;
...
State plainly that Prompt 30's static-only rule is superseded, and where.
...
the home lab by deployment choice, and that Prompt 30's static-only rule is superseded. Never
