# Prompt 30 — Deploy one instance to a homelab or local network
...
This stage depends on the container image and CI from Prompt 7, the host allow-list correction from
Prompt 9, and the in-process authentication from Prompt 20. It does not depend on Prompts 21–29: a
...
target**, not the project's CI or general test environment. Do not change Prompt 7's workflow, add a
Windows CI job, or make any test outside this stage depend on a Windows host. Prompt 7's CI keeps
...
- a Windows 11 host with Docker Desktop, its WSL 2 Linux-container backend, and the bundled Docker
  Compose plugin;
- one server container using the image produced by Prompt 7, pinned by immutable digest;
...
artifact Prompt 7 pruned and Prompt 8 audited. Note also that `GET /mcp` returns 405 by design, so
...
migration to `entra` mode is a real migration rather than a configuration change. Preserve Prompt
20's fail-closed HTTP startup, and never represent a LAN boundary, reverse proxy, VPN, or private CA
