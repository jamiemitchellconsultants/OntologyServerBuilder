# Prompt 39 — Add the explicit embedding refresh command

Implement only Prompt 3, "The refresh command (the only networked step)", from the current
`docs/step4.md` in the separate `OntologyService` repository. Prompts 37 and 38 already provide
...
entity, calculate Prompt 37's canonical embedding text and content hash. Sort entity work by stable
...
The default refresh reads the existing cache only through Prompt 37's validated cache primitives.
...
continue to use its deterministic hash-key writer from Prompt 37, but sort all refresh cache-entry
...
Prompt 37 primitives before attempting any replacement. Write it only through the fsync-safe atomic
