# Prompt 48 — Package the qualified-user MCP-registration skill

Using Prompt 35's exact tool names (`ontology_prepare_system_registration_request` and
...
This stage runs after Prompt 35 exists. It is numbered 48, after the independent audit in Prompt
47, only to avoid renumbering the already-audited Prompts 33–47 sequence; it has no dependency on
Prompts 36–47 and may be played as soon as Prompt 35 is complete.

## Read before editing

Read Prompt 35's submission tools and their exact names, `docs/registering-systems.md`, the
...
tool names in the body if this deployment's registered names differ from Prompt 35's:
...
Prompt 35 already specifies.

## Acceptance criteria

- `skills/register-supplier-mcp-server/SKILL.md` exists with valid YAML frontmatter and the exact
  content above, adapted only if this deployment's tool names differ from Prompt 35's.
